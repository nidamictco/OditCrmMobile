...
abstract class IAddLeadRepository {
  ...
  Stream<List<AddLeadModel>> watchLeads({
  required String staffId,
  required String role,
});
...
Future<bool> _queryExists(
  ...
  // ── Add to AddLeadRepository implementation ──
...
Stream<List<AddLeadModel>> watchLeads({
  ...
  // Real-time listener on the LEADS collection itself.
  // NOTE: we intentionally do NOT put orderBy('createdAt') + snapshots()
  // combined with per-doc FOLLOW_UPS reads inside the stream map, because
  // async work inside a stream map needs asyncMap, not map.
  return query
      .orderBy('createdAt', descending: true)
      .snapshots()
      .asyncMap((snap) async {
    // For every emitted snapshot (initial + any add/update/delete),
    // rebuild the full lead list including follow-ups — same shape
    // fetchLeads() already returns, so nothing downstream needs to change.
    final List<AddLeadModel> allLeads = await Future.wait(
      snap.docs.map((leadDoc) async {
        ...
        final followUps = followUpSnap.docs
            .map((fupDoc) => FollowUpModel.fromFirestore(fupDoc.data(), fupDoc.id))
            .toList();
